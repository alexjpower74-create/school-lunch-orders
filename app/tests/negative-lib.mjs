...
export function pageNegative({ dir, name, why, patches, spec, grep, project = 'chromium-390', expect = [] }) {
  ...
  if (!existsSync(path.join(base, 'worker', 'wrangler.toml'))) return stop('RESULT: CANNOT RUN — no worker/wrangler.toml to copy (is the Worker on this branch?)', 2)

  const args = ['playwright', 'test', `tests/${dir}/${spec}`, '--project', project, '--grep', escapeRegex(grep), '--workers', '1',
...
export function runAll(dir) {
  ...
  const files = readdirSync(folder).filter((f) => /^negative-.+\.mjs$/.test(f) && f !== 'negative-all.mjs').sort()
  const results = []
