...
const qtyByKey = (rows, key) =>
  rows.evaluateAll((rs, k) => Object.fromEntries(rs.map((r) => [r.dataset[k], Number(r.querySelector('.qty').textContent)])), key)
...
test('kitchen totals equal the orders', async ({ page, context, request }) => {
  ...
  expect(Object.fromEntries(apiDay.items.map((i) => [i.item_id, i.qty])), 'item totals from the API = the hand-computed numbers').toEqual(
    EXPECT_ITEMS,
  )
...
test('allergy rows come first with the red edge', async ({ page, context, request }) => {
  ...
  expect(order.slice(0, 2), 'children order: Liam (conflict) first, Noah (allergy on file) next').toEqual([
    ['ch-liam', 'conflict'],
    ['ch-noah', 'allergy'],
  ])
  ...
  expect(
    order.map(([id]) => id),
    'children order matches the API',
  ).toEqual(apiDay.children.map((c) => c.child_id))
  ...
  expect(await edge(page.locator('#children tr.child-row[data-child="ch-jack"]')), 'no red edge on a row without allergies').not.toContain(
    '248, 113, 113',
  )
...
test('kitchen tap targets, no sideways scroll, screenshots', async ({ page, context, request }, testInfo) => {
  ...
  for (const sel of [
    ...
    '#print-labels',
    '#staff-sign-out',
    'nav#staff-nav a[data-nav="kitchen"]',
  ]) {
...
test("one label per active line and Liam's names Milk", async ({ page, context, request }, testInfo) => {
  ...
  const { labels } = await (async () => {
    const r = await request.get(`/api/kitchen/labels?date=${THU}`, {
      headers: { Authorization: `Bearer ${await staffToken(request, PIN.kitchen)}`, 'X-Test-Now': '2026-09-15T13:30:00Z' },
    })
  ...
  await expect(page.locator('.label .label-allergen'), 'one allergen line per conflicting label').toHaveCount(
    labels.filter((l) => l.conflicts.length).length,
  )
...
test('a conflict names only its allergen, and the other allergies follow as "Also allergic to"', async ({ page, context, request }) => {
  ...
  await expect(page.locator('#children tr.child-row[data-child="ch-liam"]'), 'Liam has no other allergies').not.toContainText(
    'Also allergic to',
  )
...
test('at print no label cuts off its words, even for a child with every allergy', async ({ page, context, request }) => {
  ...
  const made = await api(
    ...
    'POST',
    '/api/family/children',
    { first_name: 'Zoe', class_id: 'room-3', allergies: info.allergens.map((a) => a.key) },
    bearer(owen),
  )
  ...
  await expect(zoe.locator('.label-allergen'), "the worst case's ALLERGY line in full").toHaveText(
    'ALLERGY: Eggs, Milk, Wheat and triticale and Gluten',
  )
  ...
  expect(
    [everyOther, 'More allergies: see the kitchen list'],
    "the worst case's other allergies: every one, or the words to see the kitchen list",
...
test('print emulation gives a white sheet and hides buttons', async ({ page, context, request }) => {
  ...
  const style = await allergen.evaluate((el) => {
    const cs = getComputedStyle(el)
    return { weight: Number(cs.fontWeight), border: parseFloat(cs.borderTopWidth), color: cs.color }
  })
