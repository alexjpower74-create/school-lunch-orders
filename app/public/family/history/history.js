...
function lineRow(l, tools) {
  ...
  return h(
    'li',
    { class: 'line line-row', dataset: { line: l.id, status: l.status } },
    h('span', { class: 'what' }, h('strong', {}, l.date_label), ` ${l.first_name}, ${l.item_name} ×${l.qty}`),
    l.conflicts.length
      ? h(
          'span',
          { class: 'pill allergen-pill match', dataset: { allergen: l.conflicts.join(' ') } },
          `Contains ${tools.words(l.conflicts)}`,
        )
      : null,
...
async function start() {
  if (!s) return
  const [info, fam, ledger, orders] = await Promise.all([
    getInfo(),
    familyApi('GET', '/api/family'),
    familyApi('GET', '/api/family/ledger'),
  ...
  $('ledger').replaceChildren(
    ...(ledger.entries.length ? ledger.entries.map(entryRow) : [h('li', { class: 'line-row muted' }, 'Nothing yet.')]),
  )
  const lines = [...orders.lines].sort((a, b) => (a.date < b.date ? 1 : a.date > b.date ? -1 : 0))
  $('lines').replaceChildren(
    ...(lines.length ? lines.map((l) => lineRow(l, allergenTools(info))) : [h('li', { class: 'line-row muted' }, 'No lunches yet.')]),
  )
  $('loading').hidden = true
