...
function renderItems() {
  ...
  body.replaceChildren(
    ...day.items.map((it) =>
      h(
        'tr',
        { class: it.qty === 0 ? 'zero' : null, dataset: { item: it.item_id } },
        h(
          'td',
          {},
          h('span', { class: 'item-name' }, it.name),
          it.vegetarian || it.allergens.length
            ? h(
                'span',
                { class: 'tags' },
                it.vegetarian ? h('span', { class: 'pill pill-veg' }, 'Vegetarian') : null,
                it.allergens.length ? h('span', { class: 'pill' }, `Contains ${allergenWords(info, it.allergens)}`) : null,
              )
            : null,
        ),
...
function renderChildren() {
  ...
  body.replaceChildren(
    ...rows.map((c) =>
      h(
        'tr',
        { class: 'child-row', dataset: { child: c.child_id, flag: c.flag || 'none' } },
        h('td', {}, h('strong', {}, c.first_name), h('span', { class: 'sub' }, classWords(c.class_name, c.grade))),
        h(
          'td',
          {},
          c.lines.map((l) => h('span', { class: 'lunch-line' }, `${l.item_name} ×${l.qty}`)),
        ),
        flagCell(c),
      ),
    ),
...
async function main() {
  ...
  $('#date-pick').addEventListener('change', (e) => {
    if (e.target.value) load(e.target.value, 'pick')
  })
