...
export const SAMPLE_SCHOOL = {
  school_name: 'SAMPLE Harbour Pond Elementary (demo)', sample: true, cutoff_days_before: 1, cutoff_time: '09:00',
  year_start: '2026-09-08', year_end: '2027-06-25',
  payment_instructions: 'Pay by Interac e-Transfer to lunch-orders@example.org (SAMPLE address) and put your family code in the message. ' +
    "Or send cash in a sealed envelope marked with your child's name and room.",
}

export const CLASSES = [
  ['room-k', 'Room 1', 'Kindergarten', 0], ['room-1', 'Room 2', 'Grade 1', 1], ['room-2', 'Room 4', 'Grade 2', 2],
  ['room-3', 'Room 5', 'Grade 3', 3], ['room-4', 'Room 7', 'Grade 4', 4], ['room-5', 'Room 8', 'Grade 5', 5],
...
export const ITEMS = [
  ...
  ['soup', 'Chicken noodle soup and a roll', 425, ['eggs', ...W], false, [1, 3], 1, 'Chicken, egg noodles (wheat, egg), carrots, celery, wheat roll'],
  ['wrap', 'Turkey and cheese wrap', 450, [...W, 'milk', 'mustard'], false, [1, 3], 1, 'Wheat tortilla, turkey, cheddar cheese, lettuce, mustard'],
  ['stirfry', 'Vegetable stir-fry with rice', 450, ['soy', 'sesame'], true, [3], 1,
  ...
  ['cookie', 'Oatmeal raisin cookie', 100, ['eggs', 'milk', ...W], true, [1, 2, 3, 4, 5], 1, 'Oats, wheat flour, butter, egg, raisins, brown sugar'],
].map(([id, name, price_cents, allergens, vegetarian, days, max_per_child, ingredients]) =>
  ({ id, name, price_cents, allergens, vegetarian, days, max_per_child, ingredients, active: true }))
...
export const FAMILIES = [
  { id: 'fam-1', label: 'Liam and Ava (SAMPLE)', code: 'KQ7M-4RTX', children: [
    { id: 'ch-liam', first_name: 'Liam', class_id: 'room-2', allergies: ['milk'] },
    { id: 'ch-ava', first_name: 'Ava', class_id: 'room-5', allergies: [] }] },
  { id: 'fam-2', label: 'Noah (SAMPLE)', code: 'W3PH-8JND', children: [
    { id: 'ch-noah', first_name: 'Noah', class_id: 'room-k', allergies: ['peanuts', 'tree_nuts'] }] },
  { id: 'fam-3', label: 'Emma, Jack and Chloe (SAMPLE)', code: 'C9VB-6FYE', children: [
  ...
  { id: 'fam-4', label: 'Owen (SAMPLE)', code: 'T5ZA-2GUK', children: [
    { id: 'ch-owen', first_name: 'Owen', class_id: 'room-4', allergies: [] }] },
]

// Children before families, lines before orders: the wipe order.
export const TABLES = ['sessions', 'pin_attempts', 'code_attempts', 'deliveries', 'entries', 'lines', 'orders', 'menu', 'no_school',
  'children', 'families', 'staff', 'items', 'classes', 'school']
...
export async function resetDb(db, now, today) {
  ...
  stmts.push(db.prepare(`INSERT INTO school (id, school_name, sample, payment_instructions, cutoff_days_before, cutoff_time, year_start,
    year_end) VALUES (1, ?, 1, ?, ?, ?, ?, ?)`).bind(s.school_name, s.payment_instructions, s.cutoff_days_before, s.cutoff_time, s.year_start, s.year_end))
  stmts.push(...insertMany(db, 'classes', ['id', 'name', 'grade', 'sort'], CLASSES.map((c) => [c.id, c.name, c.grade, c.sort])))
  const staffRows = []
  ...
  stmts.push(...ITEMS.map((i) => db.prepare(`INSERT INTO items (id, name, price_cents, ingredients, allergens, vegetarian, days,
    max_per_child, active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)`).bind(i.id, i.name, i.price_cents, i.ingredients, JSON.stringify(i.allergens),
    i.vegetarian ? 1 : 0, JSON.stringify(i.days), i.max_per_child)))
  stmts.push(...insertMany(db, 'no_school', ['date', 'kind', 'note', 'created_at'], NO_SCHOOL.map((n) => [n.date, n.kind, n.note, nowIso])))
  const famRows = []
  const childRows = []
