...
export function staffDate(c, _cal, fallback) {
...
export async function activeLinesFor(db, date) {
  const { results } = await db
    .prepare(`SELECT l.id AS line_id, l.child_id, l.item_id, l.qty, l.ack_allergens, i.name AS item_name,
...
export async function kitchenDay(c) {
  ...
  const items = new Map(
    menuRows.map((i) => [i.id, { item_id: i.id, name: i.name, qty: 0, allergens: parseList(i.allergens), vegetarian: !!i.vegetarian }]),
  )
  const itemRows = rows.length ? (await c.db.prepare('SELECT * FROM items').all()).results : []
  ...
  for (const r of rows) {
    ...
    children.get(r.child_id).lines.push({
      line_id: r.line_id,
      item_id: r.item_id,
  ...
  return json({
    ...
    classes: [...classes.values()]
      .filter((cl) => cl.qty >= 1)
      .sort((a, b) => a.sort - b.sort || byName(a.name, b.name))
      .map((cl) => ({
        class_id: cl.class_id,
        name: cl.name,
        grade: cl.grade,
        sort: cl.sort,
        children: cl.kids.size,
        qty: cl.qty,
...
export async function kitchenLabels(c) {
  ...
  return json({
    date,
    date_label: dateLabel(date),
    school_name: cal.school.school_name,
    sample: cal.school.sample,
    labels: rows.map((r) => ({
      line_id: r.line_id,
      first_name: r.first_name,
      class_name: r.class_name,
