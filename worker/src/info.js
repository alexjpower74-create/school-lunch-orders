...
export async function info(c) {
  ...
  return json({
    ...
    zone: TZ,
    today: c.today,
    date_label: dateLabel(c.today),
    long_label: longLabel(c.today),
    now: c.nowIso,
    time_label: timeLabel(c.now),
    next_school_day: next,
    next_school_day_label: next ? dateLabel(next) : null,
    payment_instructions: s.payment_instructions,
    cutoff_days_before: s.cutoff_days_before,
    cutoff_time: s.cutoff_time,
    cutoff_rule_label: cutoffRuleLabel(s.cutoff_days_before, s.cutoff_time),
    allergens: ALLERGENS,
  })
}
