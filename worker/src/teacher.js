...
async function classRows(db, classId, date) {
  const { results } = await db.prepare(`SELECT l.id AS line_id, l.child_id, l.qty, i.name AS item_name, i.allergens AS item_allergens,
    ...
    WHERE l.date = ? AND l.status = 'active' AND ch.class_id = ? ORDER BY ch.first_name, i.name`).bind(date, classId).all()
  const kids = new Map()
  for (const r of results) {
    const allergies = parseList(r.allergies)
    if (!kids.has(r.child_id)) {
      kids.set(r.child_id, { child_id: r.child_id, first_name: r.first_name, allergies, flag: null, state: r.state || null,
        state_label: STATE_LABELS[r.state] || 'Waiting', lines: [] })
    }
    kids.get(r.child_id).lines.push({ line_id: r.line_id, item_name: r.item_name, qty: r.qty, conflicts: conflicts(parseList(r.item_allergens), allergies) })
  }
  const children = [...kids.values()].sort((a, b) => (a.first_name < b.first_name ? -1 : a.first_name > b.first_name ? 1 : 0))
  for (const ch of children) ch.flag = flagOf(ch.allergies, ch.lines.map((l) => l.conflicts))
  const counts = {
...
export async function teacherDay(c) {
  ...
  return json({ date, date_label: dateLabel(date), is_today: date === c.today, status: st.status, status_label: st.status_label,
    no_school: noSchoolInfo(cal, date), class: classOut(cls), children, counts })
}
...
export async function teacherMark(c) {
  ...
  const has = await c.db.prepare("SELECT COUNT(*) AS n FROM lines WHERE child_id = ? AND date = ? AND status = 'active'")
    .bind(child.id, body.date).first()
  ...
  if (body.state === null) {
    ...
    await c.db.prepare(`INSERT INTO deliveries (date, child_id, state, staff_id, at) VALUES (?, ?, ?, ?, ?)
      ON CONFLICT (date, child_id) DO UPDATE SET state = excluded.state, staff_id = excluded.staff_id, at = excluded.at`)
      .bind(body.date, child.id, body.state, c.staff.id, c.nowIso).run()
  }
