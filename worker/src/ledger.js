...
export const ENTRY_KIND_WORDS = {
  order: 'Order',
  cancel: 'Cancel',
  closure: 'Closure credit',
  payment: 'Payment',
...
export async function balanceOf(db, familyId) {
  const r = await db
    .prepare('SELECT COALESCE(SUM(amount_cents), 0) AS b FROM entries WHERE family_id = ? AND voided = 0')
    .bind(familyId)
    .first()
  return r.b
}
...
export function insertEntry(db, { id, family_id, at, kind, amount_cents, label, method = null, note = '' }) {
  return db
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .bind(id, family_id, at, localDate(at), kind, amount_cents, label, method, note)
}
