...
const T = {
  ...
  apiClock: 'cut-off on a fake clock: Wed Sep 16 at 8:59 AM → 201, at 9:00 AM → 409 cutoff_passed, at the anchor → 409; Thu Sep 17 at the anchor → 201',
  ...
  storm: 'storm closure to the cent: seeded random orders, a price change and a cancelled line; preview = cancelled, credit = active totals, each balance drops by its own sum',
...
const CONTROLS = [
  {
    name: '01-cutoff-own-day', why: "cut-off compares against D's own 9:00 AM instead of the school day before",
    patches: [{ file: 'src/calendar.js', from: '  for (let i = 0; i < cal.school.cutoff_days_before; i++) {', to: '  for (let i = 0; i < 0; i++) {' }],
    args: only([T.unitDayBefore, T.apiClock]), expectRed: [T.unitDayBefore, T.apiClock],
  },
  {
    name: '02-cutoff-no-holiday-skip', why: 'cut-off stops skipping holidays and PD days',
    patches: [{ file: 'src/calendar.js', from: '    while (!isWeekday(p) || isPlannedDayOff(cal, p))', to: '    while (!isWeekday(p))' }],
    args: only([T.unitHoliday, T.apiOct13]), expectRed: [T.unitHoliday, T.apiOct13],
  },
  {
    name: '03-no-ack-required', why: 'the server stops requiring allergen_ack',
    patches: [{ file: 'src/orders.js', from: '    if (cs.length && l.allergen_ack !== true) {', to: '    if (false) {' }],
    args: ['--api-only', ...only([T.liamNoAck])], expectRed: [T.liamNoAck],
  },
  {
    name: '04-family-allergies', why: "conflicts use the whole family's allergies (a sibling's allergy blocks Ava)",
    patches: [{ file: 'src/orders.js', from: '    const cs = conflicts(item.allergens, child.allergies)',
      to: '    const cs = conflicts(item.allergens, [...children.values()].flatMap((k) => k.allergies))' }],
    args: ['--api-only', ...only([T.avaNoAck])], expectRed: [T.avaNoAck],
  },
  {
    name: '05-closure-ignores-qty', why: 'closure credit ignores qty (credits the unit price once per line)',
    patches: [{ file: 'src/admin.js', from: "'closure', -SUM(total_cents), ?", to: "'closure', -SUM(unit_price_cents), ?" }],
    args: ['--api-only', ...only([T.storm])], expectRed: [T.storm],
  },
  {
    name: '06-closure-credits-cancelled', why: 'closure also credits lines that were already cancelled',
    patches: [{ file: 'src/admin.js', from: "WHERE date = ? AND status = 'active' GROUP BY family_id",
      to: "WHERE date = ? AND status IN ('active', 'cancelled') GROUP BY family_id" }],
    args: ['--api-only', ...only([T.storm])], expectRed: [T.storm],
  },
  {
    name: '07-kitchen-counts-lines', why: 'kitchen totals count lines instead of qty',
    patches: [{ file: 'src/kitchen.js', from: '    item_count += r.qty', to: '    item_count += 1' }],
    args: ['--api-only', ...only([T.kitchen])], expectRed: [T.kitchen],
  },
  {
    name: '08-kitchen-includes-cancelled', why: 'kitchen includes cancelled (and closed) lines',
    patches: [{ file: 'src/kitchen.js', from: "WHERE l.date = ? AND l.status = 'active'`).bind(date).all()", to: 'WHERE l.date = ?`).bind(date).all()' }],
    args: ['--api-only', ...only([T.kitchen])], expectRed: [T.kitchen],
  },
  {
    name: '09-cancel-no-family-check', why: 'cancel skips the family check',
    patches: [{ file: 'src/orders.js', from: 'WHERE l.id = ? AND l.family_id = ?`).bind(id, c.family.id).first()', to: 'WHERE l.id = ?`).bind(id).first()' }],
    args: ['--api-only', ...only([T.isolation])], expectRed: [T.isolation],
  },
  {
    name: '10-kitchen-into-office', why: 'the role matrix lets kitchen into office',
    patches: [{ file: 'src/auth.js', from: "  office: ['admin'],", to: "  office: ['admin', 'kitchen']," }],
    args: ['--api-only', ...only([T.roles])], expectRed: [T.roles],
  },
  {
    name: '11-no-formula-guard', why: 'the CSV formula guard is removed',
    patches: [{ file: 'src/csv.js', from: "  if (/^[=+\\-@\\t\\r]/.test(s)) s = `'${s}`\n", to: '' }],
    args: only([T.unitCsv, T.apiCsv]), expectRed: [T.unitCsv, T.apiCsv],
  },
  {
    name: '12-no-code-rate-guard', why: 'the family-code rate guard is off',
    patches: [{ file: 'src/auth.js', from: '  await assertCodeAllowed(c)\n', to: '' }],
    args: ['--api-only', ...only([T.rate])], expectRed: [T.rate],
  },
  {
...
for (const [name, code] of results) console.log(`  ${code === 0 ? 'RED as intended' : code === 2 ? 'BREAK DID NOT APPLY' : 'STAYED GREEN'}  ${name}`)
