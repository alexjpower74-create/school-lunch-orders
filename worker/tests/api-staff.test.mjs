...
test('office: payment, adjustment and undo arithmetic to the cent', async () => {
  ...
  const pay = await call('POST', '/api/office/payments', {
    token: office,
    body: { family_id: 'fam-1', amount_cents: 250, method: 'cash', note: 'Envelope' },
  })
  ...
  const adj = await call('POST', '/api/office/adjustments', {
    token: office,
    body: { family_id: 'fam-1', amount_cents: -100, note: 'Spilled milk' },
  })
  assert.equal(adj.body.balance_cents, 250)
  assert.equal(adj.body.entry.label, 'Adjustment')
  assert.equal(
    (await call('POST', '/api/office/adjustments', { token: office, body: { family_id: 'fam-1', amount_cents: 55, note: 'Late fee' } }))
      .body.balance_cents,
    305,
  )
  const et = await call('POST', '/api/office/payments', {
    token: office,
    body: { family_id: 'fam-1', amount_cents: 1, method: 'etransfer' },
  })
...
test('office: families sorted by balance with totals, add a family (the code once), rename, detail', async () => {
  ...
  await call('POST', '/api/office/adjustments', {
    token: office,
    body: { family_id: 'fam-4', amount_cents: -300, note: 'Credit from last year' },
  })
  ...
  assert.deepEqual(
    list.families.map((f) => [f.id, f.balance_cents]),
    [
      ['fam-2', 375],
      ['fam-3', 0],
      ['fam-1', 0],
  ...
  assert.deepEqual(
    list.families[1].children.map((c) => c.first_name),
    ['Chloe', 'Emma', 'Jack'],
  )
...
test('office CSV: ledger and balances, header, -3.00 and the formula guard on a family label =SUM(A1)', async () => {
  ...
  await call('POST', '/api/office/payments', {
    token: office,
    body: { family_id: 'fam-4', amount_cents: 300, method: 'etransfer', note: '@home' },
  })
  await order(await familyToken('fam-3'), [{ child_id: 'ch-jack', date: '2026-09-17', item_id: 'mac', qty: 1 }])

  const r = await fetch(`${process.env.API_BASE}/api/office/ledger.csv`, {
    headers: { Authorization: `Bearer ${office}`, 'X-Test-Now': '2026-09-15T13:30:00Z' },
  })
  ...
  const range = await fetch(`${process.env.API_BASE}/api/office/ledger.csv?from=2026-09-16&to=2026-09-30`, {
    headers: { Authorization: `Bearer ${office}`, 'X-Test-Now': '2026-09-15T13:30:00Z' },
  })
  ...
  const b = await fetch(`${process.env.API_BASE}/api/office/balances.csv`, {
    headers: { Authorization: `Bearer ${office}`, 'X-Test-Now': '2026-09-15T13:30:00Z' },
  })
...
test('menu PUT: refused when an ordered item is left out; saved otherwise; a past day, a day off and an unknown item are refused', async () => {
  ...
  await order(fam, [
    ...
    { child_id: 'ch-liam', date: '2026-09-17', item_id: 'milk', qty: 2, allergen_ack: true },
  ])
  ...
  assert.equal(
    refused.body.error,
    '1 of Macaroni and cheese is already ordered for Thu Sep 17. Keep it on the menu, or make the day a no-school day.',
  )
  const milkOut = await put('2026-09-17', ['mac'])
  assert.equal(
    milkOut.body.error,
    '2 of White milk (250 mL) are already ordered for Thu Sep 17. Keep it on the menu, or make the day a no-school day.',
  )
  ...
  assert.deepEqual(
    week.days[3].items.map((i) => i.id),
    ['mac', 'milk', 'cookie'],
  )
  assert.equal((await put('2026-09-17', [])).status, 409)

  assert.deepEqual(
    [(await put('2026-09-14', ['apple'])).status, (await put('2026-09-14', ['apple'])).body.error],
    [409, 'That day has passed.'],
  )
...
test('menu fill: empty school days from today on get their usual items; days with items are not touched', async () => {
  ...
  assert.ok(
    before.days.every((d) => d.item_ids.length === 0),
    'past the seeded menu',
  )
  ...
  assert.deepEqual(
    fill.body.days.map((d) => d.item_ids),
    [
      ['soup', 'wrap', 'milk', 'apple', 'cookie'],
      ['chili'],
      ['soup', 'wrap', 'stirfry', 'milk', 'apple', 'cookie'],
      ['chili', 'mac', 'milk', 'apple', 'cookie'],
      ['pizza', 'fishcakes', 'milk', 'apple', 'cookie'],
    ],
  )
...
test('school settings: validation, sample never changes, and the cut-off rule follows the setting', async () => {
  ...
  const good = {
    ...
    payment_instructions: 'Cash in an envelope (SAMPLE).',
  ...
  const bads = [
    ...
    [{ school_name: 'x'.repeat(81) }, 'school_name'],
    [{ payment_instructions: '' }, 'payment_instructions'],
    [{ payment_instructions: 'x'.repeat(601) }, 'payment_instructions'],
    [{ cutoff_days_before: 6 }, 'cutoff_days_before'],
    [{ cutoff_days_before: -1 }, 'cutoff_days_before'],
    [{ cutoff_time: '9:00' }, 'cutoff_time'],
    [{ cutoff_time: '24:00' }, 'cutoff_time'],
    [{ year_start: '2026-13-01' }, 'year_start'],
    [{ year_end: '2026-09-08' }, 'year_end'],
  ]
  ...
  assert.equal(
    (await call('PUT', '/api/admin/school', { token: office, body: { ...good, cutoff_days_before: 2, cutoff_time: '14:30' } })).status,
    200,
  )
...
test('items: add, validation, and taking one off is refused while it is ordered for days to come', async () => {
  const office = await staffToken('admin')
  const item = {
    name: 'Toutons and molasses (SAMPLE)',
    price_cents: 375,
    ingredients: 'Bread dough (wheat), molasses',
  ...
  const bads = [
    [{ name: '' }, 'name'],
    [{ price_cents: 5001 }, 'price_cents'],
    [{ price_cents: -1 }, 'price_cents'],
    [{ allergens: ['nuts'] }, 'allergens'],
    [{ days: [6] }, 'days'],
    [{ days: [1, 1] }, 'days'],
    [{ max_per_child: 0 }, 'max_per_child'],
    [{ max_per_child: 11 }, 'max_per_child'],
    [{ vegetarian: 'yes' }, 'vegetarian'],
    [{ active: 1 }, 'active'],
    [{ ingredients: 'x'.repeat(401) }, 'ingredients'],
  ]
  for (const [patch, field] of bads) {
  ...
  assert.equal(
    (await get('/api/admin/menu?week=2026-09-07', office)).body.days[1].item_ids.includes('mac'),
    true,
    'past days keep their menu (Tue Sep 8)',
  )
  assert.equal(
    (
      await call('POST', '/api/family/orders', {
        token: fam,
        body: { lines: [{ child_id: 'ch-ava', date: '2026-09-22', item_id: 'mac', qty: 1 }] },
      })
    ).body.code,
    'not_on_menu',
  )
...
test('staff: a taken PIN is refused, a new PIN signs in, and the school always keeps one office PIN', async () => {
  ...
  assert.equal(
    (await call('POST', '/api/admin/staff', { token: office, body: { ...base, pin: '9999', class_id: 'room-99' } })).body.field,
    'class_id',
  )
  ...
  assert.deepEqual(
    { ...add.body.staff, id: 'x' },
    { id: 'x', name: 'Ms. Test (SAMPLE)', role: 'teacher', class_id: 'room-3', active: true },
  )
  ...
  assert.equal(
    (await call('PUT', '/api/admin/staff/st-office', { token: office, body: { ...me, role: 'admin', active: false } })).status,
    409,
  )
  const second = await call('PUT', `/api/admin/staff/${add.body.staff.id}`, {
    token: office,
    body: { ...base, role: 'admin', active: true },
  })
  assert.equal(second.status, 200)
  assert.equal(
    (await call('PUT', '/api/admin/staff/st-office', { token: office, body: { ...me, role: 'kitchen', active: true } })).status,
    200,
  )
  ...
  const pinChange = await call('PUT', '/api/admin/staff/st-kitchen', {
    token: t2,
    body: { name: 'Mr. Kean (SAMPLE)', role: 'kitchen', class_id: null, active: false },
  })
  ...
  assert.equal(
    (
      await call('PUT', '/api/admin/staff/st-oldford', {
        token: t2,
        body: { name: 'Ms. Oldford (SAMPLE)', role: 'teacher', class_id: 'room-2', active: true, pin: '9999' },
      })
    ).body.code,
    'pin_taken',
  )
...
test('classes: add and edit with validation; settings lists them with child counts', async () => {
  ...
  const edit = await call('PUT', `/api/admin/classes/${add.body.class.id}`, {
    token: office,
    body: { name: 'Room 11', grade: 'Grade 6', sort: 8 },
  })
  ...
  assert.deepEqual(settings.classes.map((c) => [c.id, c.child_count]).slice(0, 3), [
    ['room-k', 1],
    ['room-1', 1],
    ['room-2', 2],
  ])
...
test('demo scenario: the promises in docs/API.md hold (Thu Sep 24 2026, 1:30 PM)', async () => {
  ...
  assert.deepEqual(
    seed.body.families.map((f) => f.code),
    ['KQ7M-4RTX', 'W3PH-8JND', 'C9VB-6FYE', 'T5ZA-2GUK'],
  )
  ...
  assert.ok(
    liam.lines.some((l) => l.conflicts.includes('milk') && l.acknowledged),
    'Liam with an acknowledged milk item',
  )
  assert.ok(
    k.children.some((c) => c.flag === 'allergy'),
    'an allergy row',
  )
  ...
  assert.ok(ledgers['fam-1'].balance_cents < 0, `fam-1 in credit (${ledgers['fam-1'].balance_cents})`)
  assert.equal(payments('fam-1').length, 1)
  assert.equal(-payments('fam-1')[0].amount_cents % 1000, 0, 'a round e-Transfer')
  assert.equal(ledgers['fam-2'].balance_cents, 0, 'fam-2 exactly paid up')
  assert.equal(payments('fam-2')[0].method, 'etransfer')
  ...
  const signs = Object.keys(ledgers)
    .sort()
    .map((f) => Math.sign(ledgers[f].balance_cents))
  assert.deepEqual(signs, [-1, 0, 1, 1], 'the office sees one credit, one paid up and two owing')
  const office2 = (await get('/api/office/families', office, T)).body
  assert.ok(office2.totals.credit_cents < 0 && office2.totals.owing_cents > 0)
  ...
  assert.equal(
    (await get('/api/teacher/day?class_id=room-2', await staffToken('admin', '2026-09-24T12:00:00Z'), '2026-09-24T12:00:00Z')).body.counts
      .delivered,
    0,
    'nothing given out before noon',
  )
})
