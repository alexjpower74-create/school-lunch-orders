...
test('allergens: Liam + mac without allergen_ack → 409 allergen_ack_required listing that index', async () => {
  ...
  assert.equal(r.body.error, 'Liam is allergic to Milk, and Macaroni and cheese contains it. Tick "I understand" to order it anyway.')
...
test("allergens: a request mixing Liam's and Ava's mac lists only Liam's line; several allergens are named in list order", async () => {
  ...
  assert.equal(two.body.error, 'Chloe is allergic to Sesame seeds and Soy, and Vegetable stir-fry with rice contains it. Tick "I understand" to order it anyway.')
