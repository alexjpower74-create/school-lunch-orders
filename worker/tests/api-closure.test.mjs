...
const THU = [['chili', 1], ['mac', 1], ['milk', 2], ['apple', 2], ['cookie', 1]]
...
test('storm closure to the cent: seeded random orders, a price change and a cancelled line; preview = cancelled, credit = active totals, each balance drops by its own sum', async () => {
    ...
    for (const [fam, kids] of Object.entries(KIDS)) {
      ...
      for (const child_id of kids) {
        for (const [item_id, max] of THU) {
          const r = rnd()
          if (r < (round === 0 ? 0.45 : 0.3)) out[fam].push({ child_id, date: DAY, item_id, qty: max === 2 && rnd() < 0.5 ? 2 : 1, allergen_ack: true })
        }
      }
    }
    return out
  ...
  first['fam-1'].push({ child_id: 'ch-ava', date: DAY, item_id: 'mac', qty: 1 }, { child_id: 'ch-ava', date: DAY, item_id: 'milk', qty: 2 },
  ...
  const have = new Set(Object.values(first).flat().map((l) => `${l.child_id}|${l.item_id}`))
  const second = plan(1)
  second['fam-3'].push({ child_id: 'ch-jack', date: DAY, item_id: 'mac', qty: 1 })
  for (const [fam, lines] of Object.entries(second)) {
    const fresh = lines.filter((l, i, all) => !have.has(`${l.child_id}|${l.item_id}`) && all.findIndex((x) => x.child_id === l.child_id && x.item_id === l.item_id) === i)
  ...
  for (const fam of Object.keys(KIDS)) {
    ...
    before[fam] = {
      lines, balance: (await get('/api/family/ledger', tokens[fam])).body.balance_cents,
  ...
  assert.ok(allActive.some((l) => l.qty === 2), 'at least one line has qty 2')
  ...
  assert.deepEqual(post.body.day, { date: DAY, date_label: 'Thu Sep 17', kind: 'closure', kind_label: 'School closed', note: 'Storm (SAMPLE)' })
  ...
  let creditSum = 0
  for (const fam of Object.keys(KIDS)) {
    closedSum += sum((await get('/api/family/orders', tokens[fam])).body.lines.filter((l) => l.date === DAY && l.status === 'closed').map((l) => l.total_cents))
    creditSum -= sum((await get('/api/family/ledger', tokens[fam])).body.entries.filter((e) => e.kind === 'closure').map((e) => e.amount_cents))
  }
  ...
  const again = await call('POST', '/api/family/orders', { token: tokens['fam-4'], body: { lines: [{ child_id: 'ch-owen', date: DAY, item_id: 'chili', qty: 1 }] } })
  ...
  const balances = Object.fromEntries(await Promise.all(Object.keys(KIDS).map(async (f) => [f, (await get('/api/family/ledger', tokens[f])).body.balance_cents])))
  ...
  for (const fam of Object.keys(KIDS)) {
    ...
    for (const l of (await get('/api/family/orders', tokens[fam])).body.lines.filter((x) => x.date === DAY)) assert.notEqual(l.status, 'active')
  }
  assert.equal((await get(`/api/kitchen/day?date=${DAY}`, office)).body.totals.item_count, 0, 'DELETE restores no lines')
  const reorder = await call('POST', '/api/family/orders', { token: tokens['fam-4'], body: { lines: [{ child_id: 'ch-owen', date: DAY, item_id: 'chili', qty: 1 }] } })
...
test('no-school days: validation, a closure does not move the next cut-off, and a past day cannot be removed', async () => {
  ...
  for (const [body, field] of bads) {
    ...
    if (field === 'date') {
      ...
      assert.deepEqual([p.status, p.body.field, p.body.error], [400, 'date', r.body.error], `the preview refuses ${body.date} like the POST`)
    }
  }
  ...
  assert.deepEqual((await get('/api/admin/no-school/preview?date=2026-09-30', office)).body,
    { date: '2026-09-30', date_label: 'Wed Sep 30', lines: 0, item_count: 0, families: 0, credit_cents: 0 })
  ...
  assert.equal((await get('/api/family/menu?week=2026-09-28', fam)).body.days[2].cutoff_label, 'Order by 9:00 AM Mon Sep 28', 'a PD day does move it')
  ...
  assert.deepEqual(settings.no_school_days.map((d) => d.date), ['2026-09-15', '2026-09-23', '2026-09-29', '2026-10-12', '2026-10-23', '2026-11-11'])
